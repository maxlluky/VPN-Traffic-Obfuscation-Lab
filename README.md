...
- `ALPINE_TAG`: Use a valid Alpine version (e.g., `3.21.2`).
...
After cloning, you must ensure that the WireGuard client configuration files exist. These are usually generated by the WireGuard server or manually created.

Make sure the following files exist (template or specific configs):
- `services/wg-client/wireguard/wg0.conf` (for baseline)
- `services/wg-client/udp2raw/wg0.conf` (for udp2raw)
- `services/wg-client/obfs4/wg0.conf` (for obfs4)

*Note: Ensure `AllowedIPs` in the client configs matches the lab network (e.g., `172.30.30.0/24`) and the `Endpoint` points to the correct gateway or localhost tunnel.*
